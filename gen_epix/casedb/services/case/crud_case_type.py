...
def case_service_crud_case_type(
    ...
    with self.repository.uow() as uow:
        ...
        if is_metadata_admin_or_above(self, cmd.user):
            return _crud_case_type_by_admin(self, uow, cmd)
        return _crud_case_type_by_non_admin(self, uow, cmd)


def _crud_case_type_by_admin(
...
def _crud_case_type_by_non_admin(
    self: BaseCaseService,
    uow: BaseUnitOfWork,
    cmd: command.CaseTypeCrudCommand,
) -> (
