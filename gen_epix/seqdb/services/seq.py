...
class SeqService(BaseSeqService):
    ...
    def handle_file_crud(
        ...
        if is_create:
            ...
            file_content_bytes: BytesIO = BytesIO(content_bytes)

            # TODO: Finish implementation of validation of file content

            # Implementation A (Probability: 0.5)
            # Heuristic sniff first byte (ignoring whitespace) to pick FASTA ('>') or FASTQ ('@'),
            # then fully parse and validate using Biopython.
            file_content_bytes.seek(0)
            file_content_header: bytes = file_content_bytes.read(4096).lstrip()
            file_content_bytes.seek(0)
            if not file_content_header:
                raise exc.InvalidArgumentsError("File content is empty")
            first_chunk: bytes = file_content_header[:1]
            if first_chunk == b">":
                sequence_format: str = "fasta"
            elif first_chunk == b"@":
            ...
            text_stream = TextIOWrapper(
                file_content_bytes, encoding="utf-8", newline=None
            )
            try:
                sequence_records: list[Any] = list(SeqIO.parse(text_stream, sequence_format))  # type: ignore
            except Exception as e:
                raise exc.InvalidArgumentsError(
                    f"Invalid {sequence_format.upper()} content: {e}"
                ) from e

            if not sequence_records:
                raise exc.InvalidArgumentsError(
                    f"No records found in {sequence_format.upper()} file"
                )
            ...
            # fast test code to check if FileCrudCommand works
            # self.app.handle(
