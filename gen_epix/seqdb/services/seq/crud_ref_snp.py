...
def seq_service_crud_ref_snp(
    self: BaseSeqService, cmd: command.RefSnpCrudCommand
) -> list[model.RefSnp] | model.RefSnp | list[UUID] | UUID | list[bool] | bool | None:
    """Handle CRUD operations for RefSnp entities."""
    user_id = cmd.user.id if cmd.user else None
    ref_snps: list[model.RefSnp] = cmd.get_objs()  # type: ignore[assignment]
    if cmd.is_create():
