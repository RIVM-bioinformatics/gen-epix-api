...
def seq_service_crud_ref_snp_set_member(
    self: BaseSeqService, cmd: command.RefSnpSetMemberCrudCommand
) -> (
    ...
    """Handle CRUD operations for RefSnpSetMember entities."""
    user_id = cmd.user.id if cmd.user else None
    ref_snp_set_members: list[model.RefSnpSetMember] = cmd.get_objs()  # type: ignore[assignment]
    if cmd.is_create():
