...
def test_generate_sequences_happy_flow() -> None:
    ...
    assert all(isinstance(sample, model.SampleForUpload) for sample in batch.samples)


def test_generate_sequences_reproducibility() -> None:
    settings = SeqGenerationSettings(
        n_loci=5,
        locus_length=20,
        p_locus_deletion=0.01,
...
def test_generate_sequences_uniqueness() -> None:
