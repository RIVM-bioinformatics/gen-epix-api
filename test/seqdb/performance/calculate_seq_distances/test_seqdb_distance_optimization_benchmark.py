...
Profiles are generated with realistic distance distributions: per-locus
mutation probabilities sampled from Uniform(0, _LOCUS_MAX_MUTATION_PROB).
This means most pairs exceed max_stored_distance=20 (cross-strain), while
same-strain pairs cluster within the threshold — matching the production
distance distribution.

Large SQLite base files are stored in BENCHMARK_DATA_DIR (native WSL2
filesystem). The filename includes _BENCH_VERSION so changing generation
parameters automatically invalidates old cached files.
...
BENCHMARK_DATA_DIR = Path(__file__).parent
...
# pyodbc raises ODBC 07002 when a single parameterised query has more than
...
# Variants correspond to flags passed directly to _calculate_and_store_distances.
# The command layer exposes only a single use_numpy_allele_distance flag (which
# applies the gate automatically); the benchmark bypasses the gate to measure
...
VARIANTS = [
    ...
    # },
    {
        "name": "blob_numpy_batch",
        "numpy": True,
        "batch": True,
        "int32_vocab": False,
    },
    {
        "name": "blob_int32_vocab",
        "numpy": True,
        "batch": False,
        "int32_vocab": True,
    },
...
_WRITE_FNS = frozenset(
    {
        "update_some_seq_distance_content",
        "create_some",
    }
)
...
def _plot_total_duration(results: list[dict], out: Path) -> None:  # pragma: no cover
    ...
    for row_idx, n_new in enumerate(n_new_values):
        for col_idx, repo in enumerate(repos):
            ...
            _grouped_bars(ax, results, repo, n_new, n_ex_values)
...
def _plot_lines_per_n_existing(
    results: list[dict], out: Path
) -> None:  # pragma: no cover
    """Line chart: one subplot per (repo, n_existing), one line per variant."""
    if not _HAS_MATPLOTLIB:
        return
    repos = sorted({r["repo"] for r in results})
    n_new_values = sorted({r["n_new"] for r in results})
...
class TestDistanceOptimizationBenchmark:
    """Compare three distance-calculation variants on DICT and SA_SQLITE repos.

    Each variant gets a fresh copy of the repository so timings are comparable.
    ...
    Variants are exercised by calling _calculate_and_store_distances directly
    with explicit flags. This bypasses the command-layer gate (which would pick
    the variant automatically based on n_new vs _INT32_VOCAB_GATE), so each
    variant can be measured independently at every (n_existing, n_new) point.
    """
    ...
    def setup(self, env: Env) -> None:
        ...
        for n_ex in N_EXISTING:
            ...
            dist_proto, allele_proto_id, locus_set_id, _ = _extract_protocol_info(db)
    ...
    def test_variant(
        ...
        user_id = user.id
        dist_proto = self.dist_protocols[n_existing]
        ...
        # Get the service reference so we can call the internal function
        # directly — this bypasses the command-layer gate and lets us measure
        # each variant at every (n_existing, n_new) point independently.
        ...
        try:
            ...
            # Retrieve sub-benchmark using the public command (retrieve does
            # not have variant-specific flags on the current branch).
            ...
            retrieve_cmd = command.RetrieveSimilarProfilesCommand(
                user=user,
                protocol_id=cast(UUID, dist_proto.id),
...
class _MssqlBenchmarkBase:
    ...
    Each concrete subclass sets `_N_EXISTING_MSSQL`. Alphabetical ordering
    (1000 < 5000) ensures the smaller DB runs first; the larger class wipes
    and reseeds automatically when the existing profile count does not match.
    """
    ...
    def test_variant(self, env: Env, variant: dict, n_new: int) -> None:
        ...
        user = env.get_root_user()
        user_id = user.id
        dist_proto = self.dist_protocol

        self._cleanup_variant_data(user_id, new_profs, new_samps)
