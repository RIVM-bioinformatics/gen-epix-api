...
class TestOauthClientCredentialsFlow:
    ...
    @pytest.mark.scenario_ids("TC-SEC-24-01")
    def test_jwt_sensitive_info_disclosure(self, requestor_app: RequestorApp) -> None:
        """
    ...
    def test_jwt_is_using_aws_cognito(
        self, oauth_server: ServerManager, requestor_app: RequestorApp
    ) -> None:
    ...
    def test_check_jwt_if_using_asymmetric(self, requestor_app: RequestorApp) -> None:
    ...
    @pytest.mark.scenario_ids("TC-SEC-24-03")
    def test_try_changing_jwt_signing_algorithm(
        ...
        modified_token = f"{TestOauthClientCredentialsFlow._b64url_encode_no_pad(decoded_header)}.{TestOauthClientCredentialsFlow._b64url_encode_no_pad(decoded_payload)}.{signature_b64}"
    ...
    def test_try_jwt_header_injections(
        ...
        modified_token = f"{TestOauthClientCredentialsFlow._b64url_encode_no_pad(decoded_header)}.{TestOauthClientCredentialsFlow._b64url_encode_no_pad(decoded_payload)}.{jwt_token.split('.')[2]}"
    ...
    def test_verify_if_within_scope(self, requestor_app: RequestorApp) -> None:
    ...
    def test_load_keys_ignores_invalid_key(
        self, requestor_app: RequestorApp, invalid_key: dict
    ) -> None:
        ...
        assert valid_key == requestor_app.oauth_idp_client._signing_keys.get(
            "valid-kid"
        )
