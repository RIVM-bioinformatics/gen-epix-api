...
class TestCaseDBEdgeCasesRefDataAccess:
    ...
    # ------------------------------------------------------------------------------------------------
    # Metadata masking edge cases (proof of concept that policy is working in this integration setup)
    # ------------------------------------------------------------------------------------------------
    def test_case_type_contains_metadata_for_super_users(
        self, setup_case_type_data: None
    ) -> None:
        """
        Assert that case types returned to root user contain metadata fields (created_at, modified_at, modified_by),
        since superusers should not have metadata masked.
        """
        root_user = self.env.get_root_user()
    ...
    def test_case_type_should_not_contain_metadata_for_org_users(
        ...
        """
        Assert that case types returned to org users have metadata fields (created_at, modified_at, modified_by) nulled out,
        since org users should have metadata masked according to the ModelMetadataPolicy.
        """
        spec = EDGE_CASES[
            0
        ]  # Random spec since metadata masking should apply to all org users in this setup
        user = self.get_user(spec.user_name)
        ...
        for ct in result:
            ...
            assert ct.modified_at is None
            assert ct.modified_by is None
