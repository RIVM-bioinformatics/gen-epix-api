...
class BaseUploadTestCase(TestCase):
    ...
    def create_parent_for_upload(
        ...
        external_ids: list[ExternalIdentifierForUpload] | None = None,
...
class TestChildObjectProvision(BaseUploadTestCase):
    ...
    def test_2_2_parent_with_child1_only(self) -> None:
        ...
        self.assertEqual(retval.parents[0].id, created_parent_id)
        self.assertEqual(retval.parents[0].children1[0].id, created_child1_id)  # type: ignore[index,union-attr]
    ...
    def test_2_3_parent_with_child2_only(self) -> None:
        ...
        self.assertStatusCount(retval, n_created=2)
        self.assertEqual(retval.parents[0].id, created_parent_id)
        self.assertEqual(retval.parents[0].children2[0].id, created_child2_id)  # type: ignore[index,union-attr]
...
class TestReferenceDataLinks(BaseUploadTestCase):
    ...
    def test_3_4_2_child2_no_id_or_code_succeeds(self) -> None:
        ...
        self.assertBatchProcessed(retval)
        self.assertStatusCount(retval, n_created=2)
        self.assertEqual(parent.children2[0].ref2_id, None)  # type: ignore[index,union-attr]
...
class TestParentLinks(BaseUploadTestCase):
    ...
    def test_4_1_child_null_parent_id_set_during_upload(self) -> None:
        ...
        existing_ref1 = self.create_ref1(self.ref1_id, "existing_code")
        child1 = self.create_child1_for_upload(
            parent_id=NULL_ID, ref1_id=existing_ref1.id
        )
        parent = self.create_parent_for_upload(children1=[child1])
        # Set up mocks
        created_parent_id = self.generated_ids[0]
        ...
        self.assertBatchProcessed(retval)
        self.assertStatusCount(retval, n_created=2)
        self.assertEqual(parent.children1[0].parent_id, created_parent_id)  # type: ignore[index,union-attr]
    ...
    def test_4_2_1_child_parent_id_parent_not_exists_fails(self) -> None:
        ...
        existing_ref1 = self.create_ref1(self.ref1_id, "existing_code")
        child1 = self.create_child1_for_upload(
            parent_id=self.parent_id, ref1_id=existing_ref1.id
        )
        parent = self.create_parent_for_upload(
    ...
    def test_4_2_2_child_parent_id_matches_succeeds(self) -> None:
        ...
        self.assertEqual(parent.id, created_parent_id)  # type: ignore[index,union-attr]
        self.assertEqual(parent.children1[0].id, created_child1_id)  # type: ignore[index,union-attr]
...
class TestFieldMutability(BaseUploadTestCase):
    ...
    def test_5_1_3_1_always_mutable_dict_add_new_key(self) -> None:
        ...
        parent = self.create_parent_for_upload(parent_id=self.parent_id, c=new_value)
...
class TestExternalIdentifiers(BaseUploadTestCase):
    ...
    def test_6_1_no_external_ids_provided(self) -> None:
        ...
        # Create upload batch
        parent = self.create_parent_for_upload(external_ids=None)
        # Set up mocks
        created_parent_id = parent.id
        self.service.generate_id.side_effect = [
            created_parent_id,  # ID of the newly created parent
    ...
    def test_6_2_1_1_existing_external_id_null_parent_sets_id(self) -> None:
        ...
        # Create upload batch
        external_id = self.create_external_identifier_for_upload(
            identifier_issuer_id=self.identifier_issuer_id
        )
        existing_parent_id = self.generated_ids[0]
        existing_external_id = self.get_external_identifier_from_for_upload(
            external_id, internal_id=existing_parent_id
        )
        parent = self.create_parent_for_upload(external_ids=[external_id])
        ...
        self.service.app.handle.side_effect = [
            ...
            [existing_external_id],  # The existing external identifiers
        ]
        self.service.repository.crud.side_effect = [
            [True],  # Parents exist
            [existing_parent],  # Existing parents
    ...
    def test_6_2_1_2_1_existing_external_id_same_parent_succeeds(self) -> None:
        ...
        parent = self.create_parent_for_upload(
            parent_id=existing_parent_id, external_ids=[external_id]
        )
    ...
    def test_6_2_1_2_2_existing_external_id_different_parent_fails(self) -> None:
        ...
        existing_external_id = self.get_external_identifier_from_for_upload(
            external_id, internal_id=other_parent_id
        )
        parent = self.create_parent_for_upload(
            parent_id=existing_parent_id, external_ids=[external_id]
        )
    ...
    def test_6_2_2_new_external_id_creates_after_parent(self) -> None:
        """Test 6.2.2: New external identifier - should succeed."""
        # Create upload batch
        external_id = self.create_external_identifier_for_upload(
            identifier_issuer_id=self.identifier_issuer_id
        )
        parent = self.create_parent_for_upload(
            parent_id=self.parent_id, external_ids=[external_id]
        )
        # Set up mocks
        self.service.app.handle.side_effect = [
            [self.identifier_issuer],  # The identifier issuers in the external IDs
    ...
    def test_6_2_3_1_multiple_external_ids_some_existing_same_parent(self) -> None:
        ...
        # Create upload batch
        external_id1 = self.create_external_identifier_for_upload(
            external_id="ext_id_1"
        )
        external_id2 = self.create_external_identifier_for_upload(
            external_id="ext_id_2"
        )
        parent = self.create_parent_for_upload(
            external_ids=[external_id1, external_id2]
        )
        existing_external_id1 = ExternalIdentifier(
            id=uuid4(),
            **external_id1.model_dump(),
            internal_id=parent.id,  # Same parent ID
        )
        # Set up mocks
        self.service.app.handle.side_effect = [
            [self.identifier_issuer],  # The identifier issuers in the external IDs
            [existing_external_id1],  # The existing external identifiers
        ]
        # Perform upload and verify result
        retval = self.upload_batch(parent)
        self.assertBatchProcessed(retval)
...
class TestCombinedScenarios(BaseUploadTestCase):
    ...
    def test_parent_with_children_and_external_ids(self) -> None:
        ...
        parent = self.create_parent_for_upload(
            external_ids=[external_id], children1=[child1], children2=[child2]
        )
        # Set up mocks
