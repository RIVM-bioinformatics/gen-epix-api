...
def test_crud_dispatch_all_ops(parent_repo: DictRepository, parent_id: UUID) -> None:
    ...
    res_all = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.READ_ALL
    )
    assert isinstance(res_all, list)
    # READ_ONE
    res_one = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.READ_ONE,
        obj_ids=parent_id
    )
    assert isinstance(res_one, ParentModel)
    # READ_SOME
    res_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.READ_SOME,
        obj_ids=[parent_id]
    )
    ...
    res_create = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.CREATE_ONE,
        objs=created_obj
    )
    ...
    res_update = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.UPDATE_ONE,
        objs=upd_obj
    )
    ...
    res_upsert = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.UPSERT_ONE,
        objs=upsert_obj
    )
    ...
    res_del = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.DELETE_ONE,
        obj_ids=to_del
    )
    ...
    res_create_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.CREATE_SOME,
        objs=create_some_objs
    )
    ...
    res_update_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.UPDATE_SOME,
        objs=update_some_objs
    )
    ...
    res_upsert_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.UPSERT_SOME,
        objs=upsert_some_objs
    )
    ...
    res_delete_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.DELETE_SOME,
        obj_ids=[new_upsert_id]
    )
    ...
    res_exists = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.EXISTS_ONE,
        obj_ids=parent_id
    )
    assert isinstance(res_exists, bool)
    # DELETE_ALL
    res_delete_all = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.DELETE_ALL
    )
    assert isinstance(res_delete_all, list)
    # EXISTS_SOME
    res_exists_some = parent_repo.crud(
        uow, None, ParentModel, CrudOperation.EXISTS_SOME,
        obj_ids=[uuid4(), uuid4()]
    )
