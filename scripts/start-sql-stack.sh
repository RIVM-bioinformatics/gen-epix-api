...
# Safe to rerun: initialization and Alembic upgrades are idempotent, and
...
Starts the local SQL Server database, applies all Alembic migrations, and
starts the APIs. Use --idp to include the mock OIDC provider.
...
"${compose[@]}" up -d --no-deps --wait seqdb omopdb
