# test_retrieve_stats.py

> 15 nodes · cohesion 0.31

## Key Concepts

- **test_retrieve_stats.py** (22 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **retrieve_case_type_stats_profiled()** (10 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **retrieve_case_type_stats()** (9 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **get_all_case_type_ids()** (8 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **test_retrieve_case_type_stats_scaled_profiled()** (8 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **get_user_for_test()** (6 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **Env** (6 connections)
- **User** (5 connections)
- **UUID** (5 connections)
- **get_test_client()** (4 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **fixture** (3 connections)
- **parametrize** (1 connections)
- **Profiled version of retrieve_case_type_stats.** (1 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **Fetch and return the target organization user once per test module.** (1 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`
- **Fetch and return all case type IDs once per test module.** (1 connections) — `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`

## Relationships

- [CasedbTestClient](CasedbTestClient.md) (7 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (6 shared connections)
- [gen_epix/util.py](gen_epix-util.py.md) (3 shared connections)
- [DatetimeRangeFilter](DatetimeRangeFilter.md) (2 shared connections)
- [CrudOperation](CrudOperation.md) (1 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (1 shared connections)
- [casedb/domain/enum.py](casedb-domain-enum.py.md) (1 shared connections)
- [BaseCaseService](BaseCaseService.md) (1 shared connections)
- [casedb/domain/model/__init__.py](casedb-domain-model-__init__.py.md) (1 shared connections)
- [seqdb/domain/enum.py](seqdb-domain-enum.py.md) (1 shared connections)

## Source Files

- `test/casedb/performance/retrieve_stats/test_retrieve_stats.py`

## Audit Trail

- EXTRACTED: 51 (89%)
- INFERRED: 6 (11%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*