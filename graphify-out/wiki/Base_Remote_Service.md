# Base Remote Service

> 10 nodes · cohesion 0.22

## Key Concepts

- **BaseRemoteService** (9 connections) — `gen_epix/fastapp/services/remote/service.py`
- **.handle()** (3 connections) — `gen_epix/fastapp/services/remote/service.py`
- **remote/__init__.py** (2 connections) — `gen_epix/fastapp/services/remote/__init__.py`
- **.__init__()** (2 connections) — `gen_epix/fastapp/services/remote/service.py`
- **.remote_app()** (2 connections) — `gen_epix/fastapp/services/remote/service.py`
- **.use_endpoints()** (2 connections) — `gen_epix/fastapp/services/remote/service.py`
- **App** (2 connections)
- **Any** (1 connections)
- **Command** (1 connections)
- **setter** (1 connections)

## Relationships

- [FastApp Domain Registration & UserManager](FastApp_Domain_Registration_&_UserManager.md) (3 shared connections)
- [Base Service Class](Base_Service_Class.md) (1 shared connections)
- [Core App Base Class](Core_App_Base_Class.md) (1 shared connections)

## Source Files

- `gen_epix/fastapp/services/remote/__init__.py`
- `gen_epix/fastapp/services/remote/service.py`

## Audit Trail

- EXTRACTED: 13 (87%)
- INFERRED: 2 (13%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*