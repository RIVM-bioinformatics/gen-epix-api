# test_casedb_custom.py

> 16 nodes

## Key Concepts

- **test_casedb_custom.py** (18 connections) — `test/casedb/custom/test_casedb_custom.py`
- **.get_user()** (9 connections) — `test/casedb/custom/test_casedb_custom.py`
- **TestManual** (8 connections) — `test/casedb/custom/test_casedb_custom.py`
- **Env** (7 connections)
- **.test_retrieve_cases_by_query()** (6 connections) — `test/casedb/custom/test_casedb_custom.py`
- **get_test_client()** (5 connections) — `test/casedb/custom/test_casedb_custom.py`
- **skip** (5 connections)
- **.test_read_organization_access_case_policy()** (4 connections) — `test/casedb/custom/test_casedb_custom.py`
- **.test_read_organization_admin_policy()** (4 connections) — `test/casedb/custom/test_casedb_custom.py`
- **.test_read_user_case_policy()** (4 connections) — `test/casedb/custom/test_casedb_custom.py`
- **.test_retrieve_phylogenetic_tree()** (4 connections) — `test/casedb/custom/test_casedb_custom.py`
- **UUID** (2 connections)
- **fixture** (1 connections)
- **User** (1 connections)
- **# TODO: move to test_build_db** (1 connections) — `test/casedb/custom/test_casedb_custom.py`
- **# TODO: add to test_build_db** (1 connections) — `test/casedb/custom/test_casedb_custom.py`

## Relationships

- [composite.py](composite.py.md) (6 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (5 shared connections)
- [TestClient](TestClient.md) (3 shared connections)
- [CrudOperation](CrudOperation.md) (1 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (1 shared connections)
- [casedb/domain/enum.py](casedb-domain-enum.py.md) (1 shared connections)
- [casedb/domain/model/__init__.py](casedb-domain-model-__init__.py.md) (1 shared connections)

## Source Files

- `test/casedb/custom/test_casedb_custom.py`

## Audit Trail

- EXTRACTED: 47 (96%)
- INFERRED: 2 (4%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*