# .invite_user

> 3 nodes · cohesion 0.67

## Key Concepts

- **.invite_user()** (4 connections) — `gen_epix/commondb/services/remote_app.py`
- **UserInvitation** (1 connections)
- **Send a user invitation request.** (1 connections) — `gen_epix/commondb/services/remote_app.py`

## Relationships

- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (1 shared connections)
- [CommondbRemoteApp](CommondbRemoteApp.md) (1 shared connections)

## Source Files

- `gen_epix/commondb/services/remote_app.py`

## Audit Trail

- EXTRACTED: 4 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*