# CaseAbacPolicy

> 5 nodes

## Key Concepts

- **CaseAbacPolicy** (7 connections) — `gen_epix/casedb/policies/case_abac_policy.py`
- **.get_content()** (3 connections) — `gen_epix/casedb/policies/case_abac_policy.py`
- **.get_content_return_type()** (3 connections) — `gen_epix/casedb/policies/case_abac_policy.py`
- **Command** (2 connections)
- **Model** (1 connections)

## Relationships

- [CrudOperation](CrudOperation.md) (2 shared connections)
- [commondb/domain/model/__init__.py](commondb-domain-model-__init__.py.md) (1 shared connections)
- [Casedb Case Service Implementation](Casedb_Case_Service_Implementation.md) (1 shared connections)
- [BaseAbacTestCase](BaseAbacTestCase.md) (1 shared connections)
- [CaseAbac](CaseAbac.md) (1 shared connections)

## Source Files

- `gen_epix/casedb/policies/case_abac_policy.py`

## Audit Trail

- EXTRACTED: 10 (91%)
- INFERRED: 1 (9%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*