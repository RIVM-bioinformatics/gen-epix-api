# _crud_cascade_delete

> 90 nodes · cohesion 0.05

## Key Concepts

- **_crud_cascade_delete()** (45 connections) — `gen_epix/casedb/services/case/crud_common.py`
- **crud_with_access_filter()** (29 connections) — `gen_epix/casedb/services/case/crud_common.py`
- **get_ref_data_access_from_command()** (22 connections) — `gen_epix/casedb/services/case/crud_common.py`
- **crud_col.py** (18 connections) — `gen_epix/casedb/services/case/crud_col.py`
- **crud_case_type_set.py** (15 connections) — `gen_epix/casedb/services/case/crud_case_type_set.py`
- **crud_col_set_member.py** (15 connections) — `gen_epix/casedb/services/case/crud_col_set_member.py`
- **crud_ref_dim.py** (15 connections) — `gen_epix/casedb/services/case/crud_ref_dim.py`
- **crud_case_type_set_member.py** (14 connections) — `gen_epix/casedb/services/case/crud_case_type_set_member.py`
- **case_service_crud_col()** (13 connections) — `gen_epix/casedb/services/case/crud_col.py`
- **is_refdata_admin_or_above()** (13 connections) — `gen_epix/casedb/services/case/crud_common.py`
- **case_service_crud_ref_dim()** (13 connections) — `gen_epix/casedb/services/case/crud_ref_dim.py`
- **ColCrudCommand** (12 connections) — `gen_epix/casedb/domain/command/case.py`
- **CaseTypeSetCrudCommand** (11 connections) — `gen_epix/casedb/domain/command/case.py`
- **RefDimCrudCommand** (11 connections) — `gen_epix/casedb/domain/command/case.py`
- **case_service_crud_case_type_set()** (11 connections) — `gen_epix/casedb/services/case/crud_case_type_set.py`
- **_crud_case_type_set_with_abac()** (11 connections) — `gen_epix/casedb/services/case/crud_case_type_set.py`
- **case_service_crud_case_type_set_member()** (11 connections) — `gen_epix/casedb/services/case/crud_case_type_set_member.py`
- **_crud_col_with_abac()** (11 connections) — `gen_epix/casedb/services/case/crud_col.py`
- **case_service_crud_col_set_member()** (11 connections) — `gen_epix/casedb/services/case/crud_col_set_member.py`
- **_crud_col_set_member_with_abac()** (11 connections) — `gen_epix/casedb/services/case/crud_col_set_member.py`
- **_crud_ref_dim_with_abac()** (11 connections) — `gen_epix/casedb/services/case/crud_ref_dim.py`
- **_crud_case_type_set_member_with_abac()** (10 connections) — `gen_epix/casedb/services/case/crud_case_type_set_member.py`
- **_crud_col_without_abac()** (10 connections) — `gen_epix/casedb/services/case/crud_col.py`
- **CaseTypeSetMemberCrudCommand** (9 connections) — `gen_epix/casedb/domain/command/case.py`
- **ColSetMemberCrudCommand** (9 connections) — `gen_epix/casedb/domain/command/case.py`
- *... and 65 more nodes in this community*

## Relationships

- [BaseCaseService](BaseCaseService.md) (79 shared connections)
- [BaseUnitOfWork](BaseUnitOfWork.md) (24 shared connections)
- [CaseService](CaseService.md) (11 shared connections)
- [get_case_abac_from_command](get_case_abac_from_command.md) (11 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (10 shared connections)
- [CaseTypeCrudCommand](CaseTypeCrudCommand.md) (8 shared connections)
- [crud_dim.py](crud_dim.py.md) (8 shared connections)
- [crud_col_set.py](crud_col_set.py.md) (6 shared connections)
- [TestcasedbEdgeCasesRefDataAccess](TestcasedbEdgeCasesRefDataAccess.md) (3 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (3 shared connections)
- [BaseCrudTestCase](BaseCrudTestCase.md) (2 shared connections)
- [DimLike](DimLike.md) (2 shared connections)

## Source Files

- `gen_epix/casedb/domain/command/case.py`
- `gen_epix/casedb/services/case/crud_case_type_set.py`
- `gen_epix/casedb/services/case/crud_case_type_set_member.py`
- `gen_epix/casedb/services/case/crud_col.py`
- `gen_epix/casedb/services/case/crud_col_set_member.py`
- `gen_epix/casedb/services/case/crud_common.py`
- `gen_epix/casedb/services/case/crud_ref_dim.py`
- `gen_epix/casedb/services/case/service.py`

## Audit Trail

- EXTRACTED: 338 (94%)
- INFERRED: 21 (6%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*