# test/test_client/util.py

> 12 nodes · cohesion 0.26

## Key Concepts

- **test/test_client/util.py** (17 connections) — `test/test_client/util.py`
- **get_test_root_output_dir()** (16 connections) — `test/test_client/util.py`
- **get_test_name()** (14 connections) — `test/test_client/util.py`
- **test/fastapp/util.py** (8 connections) — `test/fastapp/util.py`
- **linter.py** (4 connections) — `test/test_client/linter.py`
- **generate_hex_strings()** (4 connections) — `test/test_client/util.py`
- **generate_uuids()** (4 connections) — `test/test_client/util.py`
- **Path** (4 connections)
- **.other_general_generate_hex_strings()** (2 connections) — `run.py`
- **.other_general_generate_uuids()** (2 connections) — `run.py`
- **Enum** (2 connections)
- **This module contains the `Linter` class which is used to enforce code quality…** (1 connections) — `test/test_client/linter.py`

## Relationships

- [commondb/domain/enum.py](commondb-domain-enum.py.md) (12 shared connections)
- [AppCfg](AppCfg.md) (10 shared connections)
- [test_fastapp_rbac_service.py](test_fastapp_rbac_service.py.md) (7 shared connections)
- [.get_test_client](get_test_client.md) (4 shared connections)
- [ServiceTestClient](ServiceTestClient.md) (3 shared connections)
- [Run](Run.md) (2 shared connections)
- [Linter](Linter.md) (2 shared connections)
- [seqdb_test_client.py](seqdb_test_client.py.md) (2 shared connections)
- [SeqdbTestClient](SeqdbTestClient.md) (1 shared connections)
- [log_parser_v2.py](log_parser_v2.py.md) (1 shared connections)

## Source Files

- `run.py`
- `test/fastapp/util.py`
- `test/test_client/linter.py`
- `test/test_client/util.py`

## Audit Trail

- EXTRACTED: 61 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*