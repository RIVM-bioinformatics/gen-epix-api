# test_casedb_crud_common.py

> 46 nodes

## Key Concepts

- **test_casedb_crud_common.py** (22 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **TestCrudWithAccessFilter** (14 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **DummyCmd** (13 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **EqualsStringFilter** (12 connections) — `gen_epix/filter/equals_string.py`
- **TestCommandCategoryChecks** (11 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **TestRoleChecks** (9 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **DummyEntity** (6 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **DummyLink** (6 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **TestGetCaseAbacFromCommand** (6 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.__init__()** (4 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_with_base_class_mapping_populates_subclass()** (4 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_with_matching_link_and_none_obj_ids()** (4 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_with_matching_link_and_obj_ids_uses_compose_filter()** (4 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_with_non_matching_links_does_nothing()** (4 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **scenario_ids** (4 connections)
- **.setup_method()** (3 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_no_access_filter_provided_keeps_original()** (3 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_sets_composite_filter_and_restores_original()** (3 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_sets_filter_when_original_is_none()** (3 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **UUID** (3 connections)
- **.get_obj_ids()** (2 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_not_invoked_when_not_delete()** (2 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.test_cascade_with_unknown_model_sets_empty_mapping()** (2 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **.setup_method()** (2 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- **DataCmd** (1 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`
- *... and 21 more nodes in this community*

## Relationships

- [composite.py](composite.py.md) (6 shared connections)
- [BaseCrudTestCase](BaseCrudTestCase.md) (5 shared connections)
- [CrudOperation](CrudOperation.md) (5 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (3 shared connections)
- [CompositeFilter](CompositeFilter.md) (2 shared connections)
- [Filter](Filter.md) (2 shared connections)
- [Casedb Case Service Implementation](Casedb_Case_Service_Implementation.md) (1 shared connections)

## Source Files

- `gen_epix/filter/equals_string.py`
- `test/casedb/unit/services/case/crud/test_casedb_crud_common.py`

## Audit Trail

- EXTRACTED: 94 (98%)
- INFERRED: 2 (2%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*