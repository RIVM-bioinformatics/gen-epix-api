# BaseCrudTestCase

> 55 nodes · cohesion 0.06

## Key Concepts

- **BaseCrudTestCase** (25 connections) — `test/casedb/unit/services/case/base.py`
- **case_service_crud_case_set()** (23 connections) — `gen_epix/casedb/services/case/crud_case_set.py`
- **crud_case_set.py** (21 connections) — `gen_epix/casedb/services/case/crud_case_set.py`
- **test_casedb_crud_case_set.py** (17 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **_crud_case_set_with_abac()** (11 connections) — `gen_epix/casedb/services/case/crud_case_set.py`
- **CaseSetCrudCommand** (10 connections) — `gen_epix/casedb/domain/command/case.py`
- **_crud_case_set_without_abac()** (9 connections) — `gen_epix/casedb/services/case/crud_case_set.py`
- **_validate_case_set_deletion()** (8 connections) — `gen_epix/casedb/services/case/crud_case_set.py`
- **scenario_ids** (7 connections)
- **TestReadOperations** (7 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **.crud_case_set()** (6 connections) — `gen_epix/casedb/services/case/service.py`
- **.create_crud_command()** (6 connections) — `test/casedb/unit/services/case/base.py`
- **TestDeleteSomeOperation** (6 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **UUID** (5 connections)
- **TestAbacCreateOperation** (5 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **TestAbacNoPolicy** (5 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **TestAdminPath** (5 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **TestDeleteAllOperation** (5 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **TestUpdateOperation** (5 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **BaseCaseService** (4 connections)
- **._make_uow()** (4 connections) — `test/casedb/unit/services/case/base.py`
- **CaseSet** (3 connections)
- **.setup_method()** (3 connections) — `test/casedb/unit/services/case/base.py`
- **.test_delete_some_allowed_calls_crud()** (3 connections) — `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`
- **.create_case_abac()** (2 connections) — `test/casedb/unit/services/case/base.py`
- *... and 30 more nodes in this community*

## Relationships

- [BaseCaseService](BaseCaseService.md) (13 shared connections)
- [CrudOperation](CrudOperation.md) (6 shared connections)
- [BaseUnitOfWork](BaseUnitOfWork.md) (6 shared connections)
- [test_casedb_crud_common.py](test_casedb_crud_common.py.md) (5 shared connections)
- [get_case_abac_from_command](get_case_abac_from_command.md) (4 shared connections)
- [CaseService](CaseService.md) (3 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (3 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (2 shared connections)
- [_crud_cascade_delete](_crud_cascade_delete.md) (2 shared connections)
- [casedb/domain/enum.py](casedb-domain-enum.py.md) (2 shared connections)
- [DimLike](DimLike.md) (2 shared connections)
- [case_service_crud_ref_col](case_service_crud_ref_col.md) (2 shared connections)

## Source Files

- `gen_epix/casedb/domain/command/case.py`
- `gen_epix/casedb/services/case/crud_case_set.py`
- `gen_epix/casedb/services/case/service.py`
- `test/casedb/unit/services/case/base.py`
- `test/casedb/unit/services/case/crud/test_casedb_crud_case_set.py`

## Audit Trail

- EXTRACTED: 145 (96%)
- INFERRED: 6 (4%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*