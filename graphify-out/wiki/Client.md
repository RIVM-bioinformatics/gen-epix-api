# Client

> 125 nodes

## Key Concepts

- **Client** (53 connections) — `test/test_client/oauth/client_store.py`
- **OAuth2Validator** (35 connections) — `test/test_client/oauth/validators.py`
- **TokenStore** (33 connections) — `test/test_client/oauth/token_store.py`
- **ClientStore** (30 connections) — `test/test_client/oauth/client_store.py`
- **TestClient** (22 connections) — `test/test_client/oauth/test_client_store.py`
- **oauth/__init__.py** (14 connections) — `test/test_client/oauth/__init__.py`
- **test_validators.py** (13 connections) — `test/test_client/oauth/test_validators.py`
- **TestOAuth2ValidatorIntegration** (12 connections) — `test/test_client/oauth/test_validators.py`
- **validators.py** (10 connections) — `test/test_client/oauth/validators.py`
- **token_store.py** (9 connections) — `test/test_client/oauth/token_store.py`
- **TestClientStoreIntegration** (8 connections) — `test/test_client/oauth/test_client_store.py`
- **client_store.py** (8 connections) — `test/test_client/oauth/client_store.py`
- **test_client_store.py** (8 connections) — `test/test_client/oauth/test_client_store.py`
- **.setup_method()** (7 connections) — `test/test_client/oauth/test_validators.py`
- **.test_error_handling_and_edge_cases()** (6 connections) — `test/test_client/oauth/test_validators.py`
- **.get_client()** (5 connections) — `test/test_client/oauth/client_store.py`
- **.setup_method()** (5 connections) — `test/test_client/oauth/test_validators.py`
- **._hash_secret()** (4 connections) — `test/test_client/oauth/client_store.py`
- **.create_client()** (4 connections) — `test/test_client/oauth/client_store.py`
- **.store_client()** (4 connections) — `test/test_client/oauth/client_store.py`
- **.update_client()** (4 connections) — `test/test_client/oauth/client_store.py`
- **.setup_method()** (4 connections) — `test/test_client/oauth/test_client_store.py`
- **.test_redirect_uri_validation_comprehensive()** (4 connections) — `test/test_client/oauth/test_validators.py`
- **.test_scope_validation_edge_cases()** (4 connections) — `test/test_client/oauth/test_validators.py`
- **.__init__()** (4 connections) — `test/test_client/oauth/validators.py`
- *... and 100 more nodes in this community*

## Relationships

- [OAuth Validators (Test Client)](OAuth_Validators_Test_Client.md) (19 shared connections)
- [MockRequest](MockRequest.md) (14 shared connections)
- [Token](Token.md) (13 shared connections)
- [server.py](server.py.md) (12 shared connections)
- [.get_token](get_token.md) (8 shared connections)
- [TestClientStore](TestClientStore.md) (7 shared connections)
- [TestTokenStore](TestTokenStore.md) (3 shared connections)
- [JWKSManager](JWKSManager.md) (3 shared connections)
- [RemoteApp](RemoteApp.md) (2 shared connections)
- [CrudOperation](CrudOperation.md) (2 shared connections)
- [composite.py](composite.py.md) (1 shared connections)
- [InMemoryOrganizationRepository](InMemoryOrganizationRepository.md) (1 shared connections)

## Source Files

- `test/test_client/oauth/__init__.py`
- `test/test_client/oauth/client_store.py`
- `test/test_client/oauth/test_client_store.py`
- `test/test_client/oauth/test_validators.py`
- `test/test_client/oauth/token_store.py`
- `test/test_client/oauth/validators.py`

## Audit Trail

- EXTRACTED: 255 (92%)
- INFERRED: 23 (8%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*