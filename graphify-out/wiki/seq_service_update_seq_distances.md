# seq_service_update_seq_distances

> 7 nodes

## Key Concepts

- **seq_service_update_seq_distances()** (13 connections) — `gen_epix/seqdb/services/seq/calculate_seq_distance.py`
- **UpdateSeqDistancesCommand** (8 connections) — `gen_epix/seqdb/domain/command/seq.py`
- **.update_seq_distances()** (4 connections) — `gen_epix/seqdb/domain/service/seq.py`
- **.update_seq_distances()** (4 connections) — `gen_epix/seqdb/services/seq/service.py`
- **For a given distance protocol, find all profiles that don't yet have a…** (1 connections) — `gen_epix/seqdb/domain/command/seq.py`
- **Update sequence distance calculations.** (1 connections) — `gen_epix/seqdb/domain/service/seq.py`
- **For a given distance protocol, find all profiles that don't yet have a…** (1 connections) — `gen_epix/seqdb/services/seq/calculate_seq_distance.py`

## Relationships

- [seqdb/domain/model/__init__.py](seqdb-domain-model-__init__.py.md) (3 shared connections)
- [BaseSeqService](BaseSeqService.md) (3 shared connections)
- [calculate_seq_distance.py](calculate_seq_distance.py.md) (3 shared connections)
- [seq_service_calculate_seq_distances_for_new_profiles](seq_service_calculate_seq_distances_for_new_profiles.md) (2 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (1 shared connections)
- [SeqdbRemoteApp](SeqdbRemoteApp.md) (1 shared connections)
- [command/seq.py](command-seq.py.md) (1 shared connections)
- [test_seqdb_retrieve_best.py](test_seqdb_retrieve_best.py.md) (1 shared connections)
- [composite.py](composite.py.md) (1 shared connections)
- [test_seqdb_calculate_seq_distance.py](test_seqdb_calculate_seq_distance.py.md) (1 shared connections)
- [SeqService](SeqService.md) (1 shared connections)

## Source Files

- `gen_epix/seqdb/domain/command/seq.py`
- `gen_epix/seqdb/domain/service/seq.py`
- `gen_epix/seqdb/services/seq/calculate_seq_distance.py`
- `gen_epix/seqdb/services/seq/service.py`

## Audit Trail

- EXTRACTED: 25 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*