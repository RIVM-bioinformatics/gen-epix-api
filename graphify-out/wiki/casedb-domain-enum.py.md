# casedb/domain/enum.py

> 54 nodes · cohesion 0.06

## Key Concepts

- **casedb/domain/enum.py** (105 connections) — `gen_epix/casedb/domain/enum.py`
- **ServiceType** (24 connections) — `gen_epix/casedb/domain/enum.py`
- **Enum** (19 connections)
- **casedb/domain/service/__init__.py** (17 connections) — `gen_epix/casedb/domain/service/__init__.py`
- **casedb/domain/policy/permission.py** (11 connections) — `gen_epix/casedb/domain/policy/permission.py`
- **service/case.py** (11 connections) — `gen_epix/casedb/domain/service/case.py`
- **casedb/domain/service/abac.py** (10 connections) — `gen_epix/casedb/domain/service/abac.py`
- **BaseAbacService** (10 connections) — `gen_epix/casedb/domain/service/abac.py`
- **service/geo.py** (8 connections) — `gen_epix/casedb/domain/service/geo.py`
- **service/seqdb.py** (8 connections) — `gen_epix/casedb/domain/service/seqdb.py`
- **BaseGeoService** (7 connections) — `gen_epix/casedb/domain/service/geo.py`
- **service/ontology.py** (7 connections) — `gen_epix/casedb/domain/service/ontology.py`
- **ConceptRelationType** (6 connections) — `gen_epix/casedb/domain/enum.py`
- **RegionRelationType** (6 connections) — `gen_epix/casedb/domain/enum.py`
- **RegionRelation** (6 connections) — `gen_epix/casedb/domain/model/geo.py`
- **BaseOntologyService** (6 connections) — `gen_epix/casedb/domain/service/ontology.py`
- **ConceptSetType** (5 connections) — `gen_epix/casedb/domain/enum.py`
- **services/geo.py** (5 connections) — `gen_epix/casedb/services/geo.py`
- **CaseRightSet** (4 connections) — `gen_epix/casedb/domain/enum.py`
- **ColTypeSet** (4 connections) — `gen_epix/casedb/domain/enum.py`
- **.get_case_abac()** (4 connections) — `gen_epix/casedb/domain/service/abac.py`
- **.get_ref_data_access()** (4 connections) — `gen_epix/casedb/domain/service/abac.py`
- **GeoService** (4 connections) — `gen_epix/casedb/services/geo.py`
- **CaseTypeSetCategoryPurpose** (3 connections) — `gen_epix/casedb/domain/enum.py`
- **._serialize_relation()** (3 connections) — `gen_epix/casedb/domain/model/geo.py`
- *... and 29 more nodes in this community*

## Relationships

- [commondb/domain/enum.py](commondb-domain-enum.py.md) (17 shared connections)
- [BaseCaseService](BaseCaseService.md) (16 shared connections)
- [casedb/domain/model/__init__.py](casedb-domain-model-__init__.py.md) (15 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (10 shared connections)
- [CrudOperation](CrudOperation.md) (9 shared connections)
- [DatetimeRangeFilter](DatetimeRangeFilter.md) (6 shared connections)
- [services/user_manager.py](services-user_manager.py.md) (6 shared connections)
- [CaseTypeAccessAbac](CaseTypeAccessAbac.md) (5 shared connections)
- [model/ontology.py](model-ontology.py.md) (5 shared connections)
- [Any](Any.md) (5 shared connections)
- [OrganizationService](OrganizationService.md) (5 shared connections)
- [CrudEndpointGenerator](CrudEndpointGenerator.md) (4 shared connections)

## Source Files

- `gen_epix/casedb/domain/enum.py`
- `gen_epix/casedb/domain/model/geo.py`
- `gen_epix/casedb/domain/policy/permission.py`
- `gen_epix/casedb/domain/service/__init__.py`
- `gen_epix/casedb/domain/service/abac.py`
- `gen_epix/casedb/domain/service/case.py`
- `gen_epix/casedb/domain/service/geo.py`
- `gen_epix/casedb/domain/service/ontology.py`
- `gen_epix/casedb/domain/service/seqdb.py`
- `gen_epix/casedb/services/geo.py`
- `gen_epix/casedb/services/ontology.py`
- `gen_epix/casedb/services/rbac.py`
- `test/casedb/unit/domain/model/test_casedb_model_complete_case_type.py`

## Audit Trail

- EXTRACTED: 243 (92%)
- INFERRED: 21 (8%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*