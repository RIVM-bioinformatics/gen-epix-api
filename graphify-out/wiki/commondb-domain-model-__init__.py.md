# commondb/domain/model/__init__.py

> 174 nodes · cohesion 0.02

## Key Concepts

- **commondb/domain/model/__init__.py** (105 connections) — `gen_epix/commondb/domain/model/__init__.py`
- **AppImplDetails** (70 connections) — `gen_epix/commondb/app_impl_details.py`
- **app_impl_details.py** (35 connections) — `gen_epix/commondb/app_impl_details.py`
- **commondb/policies/read_user_policy.py** (25 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **BaseAbacService** (23 connections) — `gen_epix/commondb/domain/service/abac.py`
- **BaseUserManager** (22 connections) — `gen_epix/fastapp/user_manager.py`
- **test_read_organization_results_only_policy.py** (20 connections) — `test/commondb/unit/policies/test_read_organization_results_only_policy.py`
- **UpdateUserPolicy** (19 connections) — `gen_epix/commondb/policies/update_user_policy.py`
- **services/system.py** (17 connections) — `gen_epix/commondb/services/system.py`
- **casedb/policies/__init__.py** (16 connections) — `gen_epix/casedb/policies/__init__.py`
- **commondb/policies/__init__.py** (16 connections) — `gen_epix/commondb/policies/__init__.py`
- **IsOrganizationAdminPolicy** (15 connections) — `gen_epix/commondb/policies/is_organization_admin_policy.py`
- **commondb/policies/read_organization_results_only_policy.py** (14 connections) — `gen_epix/commondb/policies/read_organization_results_only_policy.py`
- **ReadOrganizationResultsOnlyPolicy** (14 connections) — `gen_epix/commondb/policies/read_organization_results_only_policy.py`
- **commondb/policies/update_user_policy.py** (14 connections) — `gen_epix/commondb/policies/update_user_policy.py`
- **commondb/services/rbac.py** (14 connections) — `gen_epix/commondb/services/rbac.py`
- **commondb/policies/is_organization_admin_policy.py** (13 connections) — `gen_epix/commondb/policies/is_organization_admin_policy.py`
- **ModelMetadataPolicy** (13 connections) — `gen_epix/commondb/policies/model_metadata_policy.py`
- **commondb/policies/read_self_results_only_policy.py** (13 connections) — `gen_epix/commondb/policies/read_self_results_only_policy.py`
- **commondb/domain/service/abac.py** (12 connections) — `gen_epix/commondb/domain/service/abac.py`
- **model_metadata_policy.py** (11 connections) — `gen_epix/commondb/policies/model_metadata_policy.py`
- **omopdb/policies/__init__.py** (11 connections) — `gen_epix/omopdb/policies/__init__.py`
- **has_system_outage_policy.py** (10 connections) — `gen_epix/commondb/policies/has_system_outage_policy.py`
- **ReadSelfResultsOnlyPolicy** (9 connections) — `gen_epix/commondb/policies/read_self_results_only_policy.py`
- **User** (9 connections)
- *... and 149 more nodes in this community*

## Relationships

- [CrudOperation](CrudOperation.md) (64 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (28 shared connections)
- [CrudEndpointGenerator](CrudEndpointGenerator.md) (25 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (24 shared connections)
- [services/user_manager.py](services-user_manager.py.md) (17 shared connections)
- [Policy](Policy.md) (15 shared connections)
- [seqdb/domain/model/__init__.py](seqdb-domain-model-__init__.py.md) (15 shared connections)
- [OrganizationService](OrganizationService.md) (12 shared connections)
- [api/case.py](api-case.py.md) (10 shared connections)
- [.create_crud_cmd](create_crud_cmd.md) (8 shared connections)
- [ReadUserPolicy](ReadUserPolicy.md) (7 shared connections)
- [Permission](Permission.md) (7 shared connections)

## Source Files

- `gen_epix/casedb/policies/__init__.py`
- `gen_epix/casedb/policies/is_organization_admin_policy.py`
- `gen_epix/casedb/policies/read_organization_results_only_policy.py`
- `gen_epix/casedb/policies/read_self_results_only_policy.py`
- `gen_epix/commondb/app_impl_details.py`
- `gen_epix/commondb/domain/model/__init__.py`
- `gen_epix/commondb/domain/policy/system.py`
- `gen_epix/commondb/domain/service/abac.py`
- `gen_epix/commondb/policies/__init__.py`
- `gen_epix/commondb/policies/has_system_outage_policy.py`
- `gen_epix/commondb/policies/is_organization_admin_policy.py`
- `gen_epix/commondb/policies/model_metadata_policy.py`
- `gen_epix/commondb/policies/read_organization_results_only_policy.py`
- `gen_epix/commondb/policies/read_self_results_only_policy.py`
- `gen_epix/commondb/policies/read_user_policy.py`
- `gen_epix/commondb/policies/update_user_policy.py`
- `gen_epix/commondb/services/rbac.py`
- `gen_epix/commondb/services/system.py`
- `gen_epix/fastapp/user_manager.py`
- `gen_epix/omopdb/policies/__init__.py`

## Audit Trail

- EXTRACTED: 589 (94%)
- INFERRED: 37 (6%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*