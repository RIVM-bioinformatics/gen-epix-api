# ModelFieldProps

> 14 nodes

## Key Concepts

- **ModelFieldProps** (16 connections) — `gen_epix/fastapp/model.py`
- **complete_stored_model_field_props()** (9 connections) — `gen_epix/commondb/domain/util.py`
- **._validate_state()** (3 connections) — `gen_epix/fastapp/model.py`
- **.is_mutable_value()** (3 connections) — `gen_epix/fastapp/model.py`
- **._validate_model()** (3 connections) — `gen_epix/fastapp/model.py`
- **._validate_state()** (3 connections) — `gen_epix/fastapp/model.py`
- **model_validator** (3 connections)
- **Self** (3 connections)
- **Model** (2 connections)
- **Any** (1 connections)
- **BaseModel** (1 connections)
- **Complete the stored_model_field_props with default props for all other…** (1 connections) — `gen_epix/commondb/domain/util.py`
- **Additional properties of a model field. The application of these properties…** (1 connections) — `gen_epix/fastapp/model.py`
- **Determine if a stored value for this field is mutable.** (1 connections) — `gen_epix/fastapp/model.py`

## Relationships

- [CrudOperation](CrudOperation.md) (4 shared connections)
- [BasePersonUploadTestCase](BasePersonUploadTestCase.md) (2 shared connections)
- [casedb/domain/model/__init__.py](casedb-domain-model-__init__.py.md) (2 shared connections)
- [commondb/domain/model/__init__.py](commondb-domain-model-__init__.py.md) (2 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (2 shared connections)
- [omopdb/domain/model/__init__.py](omopdb-domain-model-__init__.py.md) (2 shared connections)
- [seqdb/domain/model/__init__.py](seqdb-domain-model-__init__.py.md) (2 shared connections)
- [BaseUnitOfWork](BaseUnitOfWork.md) (1 shared connections)
- [test_omopdb_upload.py](test_omopdb_upload.py.md) (1 shared connections)
- [CrudCommand](CrudCommand.md) (1 shared connections)
- [Policy](Policy.md) (1 shared connections)

## Source Files

- `gen_epix/commondb/domain/util.py`
- `gen_epix/fastapp/model.py`

## Audit Trail

- EXTRACTED: 34 (97%)
- INFERRED: 1 (3%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*