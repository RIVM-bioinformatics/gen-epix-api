# .create_child1_for_upload

> 35 nodes · cohesion 0.09

## Key Concepts

- **.create_child1_for_upload()** (23 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.expectBatchFailed()** (19 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.create_ref1()** (18 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test3ReferenceDataLinks** (12 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_child2_with_identifiers_and_parent_relationship()** (10 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_parent_with_children_and_identifiers()** (10 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_complex_reference_resolution()** (9 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_update_existing_parent_with_new_children()** (9 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_1_2_ref_id_provided_and_found_succeeds()** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_2_2_ref_code_provided_and_found_sets_id()** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_3_1_ref_id_and_code_mismatch_fails()** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_3_2_ref_id_and_code_match_succeeds()** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_4_2_1_child_parent_id_parent_not_exists_fails()** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **TestCombinedScenarios** (8 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_1_1_ref_id_provided_not_found_fails()** (7 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_2_1_ref_code_provided_not_found_fails()** (7 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_3_4_1_child2_null_id_no_code_provided_fails()** (7 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **.test_dry_run_and_real_run_report_same_failure()** (5 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test combinations of different scenarios.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test parent with both children and Identifiers.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test updating an existing parent with new child objects.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test complex reference data resolution across multiple children.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Test Child2 with Identifiers in combination with parent relationships and other…** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **verify_only=True and verify_only=False must agree on batch outcome.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- **Create a test child1 for upload.** (1 connections) — `test/commondb/unit/upload/test_commondb_upload.py`
- *... and 10 more nodes in this community*

## Relationships

- [.create_parent_for_upload](create_parent_for_upload.md) (53 shared connections)
- [.create_child2_for_upload](create_child2_for_upload.md) (11 shared connections)
- [BaseUploadTestCase](BaseUploadTestCase.md) (9 shared connections)
- [test_commondb_upload.py](test_commondb_upload.py.md) (7 shared connections)
- [UUID](UUID.md) (6 shared connections)
- [TestUploadEdgeCases](TestUploadEdgeCases.md) (4 shared connections)
- [Test6Identifiers](Test6Identifiers.md) (2 shared connections)
- [BaseUnitOfWork](BaseUnitOfWork.md) (1 shared connections)

## Source Files

- `test/commondb/unit/upload/test_commondb_upload.py`

## Audit Trail

- EXTRACTED: 147 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*