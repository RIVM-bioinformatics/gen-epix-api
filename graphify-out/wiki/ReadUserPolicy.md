# ReadUserPolicy

> 27 nodes · cohesion 0.13

## Key Concepts

- **ReadUserPolicy** (24 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **.filter()** (8 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **User** (7 connections)
- **.get_admin_user_ids_own_organization()** (7 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **UUID** (6 connections)
- **._get_org_and_user_ids()** (6 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **._get_admin_user_ids()** (5 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **._filter_read_one()** (4 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **._filter_read_some()** (4 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **omopdb/policies/read_user_policy.py** (4 connections) — `gen_epix/omopdb/policies/read_user_policy.py`
- **seqdb/policies/read_user_policy.py** (4 connections) — `gen_epix/seqdb/policies/read_user_policy.py`
- **._check_invalid_filter_input()** (3 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **.__init__()** (3 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **._is_no_abac_user()** (3 connections) — `gen_epix/commondb/policies/read_user_policy.py`
- **ReadUserPolicy** (3 connections) — `gen_epix/omopdb/policies/read_user_policy.py`
- **.__init__()** (3 connections) — `gen_epix/omopdb/policies/read_user_policy.py`
- **ReadUserPolicy** (3 connections) — `gen_epix/seqdb/policies/read_user_policy.py`
- **.__init__()** (3 connections) — `gen_epix/seqdb/policies/read_user_policy.py`
- **Command** (2 connections)
- **Any** (1 connections)
- **BaseAbacService** (1 connections)
- **Any** (1 connections)
- **BaseAbacService** (1 connections)
- **CommonReadUserPolicy** (1 connections)
- **Any** (1 connections)
- *... and 2 more nodes in this community*

## Relationships

- [commondb/domain/model/__init__.py](commondb-domain-model-__init__.py.md) (7 shared connections)
- [Role](Role.md) (3 shared connections)
- [composite.py](composite.py.md) (3 shared connections)
- [CompositeFilter](CompositeFilter.md) (2 shared connections)
- [UuidSetFilter](UuidSetFilter.md) (2 shared connections)
- [Policy](Policy.md) (1 shared connections)
- [CrudOperation](CrudOperation.md) (1 shared connections)
- [omopdb/domain/command/__init__.py](omopdb-domain-command-__init__.py.md) (1 shared connections)
- [services/user_manager.py](services-user_manager.py.md) (1 shared connections)
- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (1 shared connections)

## Source Files

- `gen_epix/commondb/policies/read_user_policy.py`
- `gen_epix/omopdb/policies/read_user_policy.py`
- `gen_epix/seqdb/policies/read_user_policy.py`

## Audit Trail

- EXTRACTED: 61 (92%)
- INFERRED: 5 (8%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*