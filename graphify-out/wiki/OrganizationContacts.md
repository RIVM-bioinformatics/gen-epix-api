# OrganizationContacts

> 7 nodes · cohesion 0.29

## Key Concepts

- **OrganizationContacts** (7 connections) — `gen_epix/commondb/domain/model/organization.py`
- **.retrieve_organization_contacts()** (4 connections) — `gen_epix/commondb/domain/service/organization.py`
- **.retrieve_organization_contacts()** (4 connections) — `gen_epix/commondb/services/remote_app.py`
- **.retrieve_organization_contacts()** (3 connections) — `gen_epix/commondb/services/organization.py`
- **BaseModel** (2 connections)
- **Retrieve organization contact information.** (1 connections) — `gen_epix/commondb/domain/service/organization.py`
- **Retrieve contact information for an organization.** (1 connections) — `gen_epix/commondb/services/remote_app.py`

## Relationships

- [casedb/domain/command/__init__.py](casedb-domain-command-__init__.py.md) (3 shared connections)
- [commondb/domain/model/__init__.py](commondb-domain-model-__init__.py.md) (1 shared connections)
- [commondb/domain/enum.py](commondb-domain-enum.py.md) (1 shared connections)
- [seqdb/domain/model/__init__.py](seqdb-domain-model-__init__.py.md) (1 shared connections)
- [IdentifierForUpload](IdentifierForUpload.md) (1 shared connections)
- [services/user_manager.py](services-user_manager.py.md) (1 shared connections)
- [OrganizationService](OrganizationService.md) (1 shared connections)
- [CommondbRemoteApp](CommondbRemoteApp.md) (1 shared connections)

## Source Files

- `gen_epix/commondb/domain/model/organization.py`
- `gen_epix/commondb/domain/service/organization.py`
- `gen_epix/commondb/services/organization.py`
- `gen_epix/commondb/services/remote_app.py`

## Audit Trail

- EXTRACTED: 16 (100%)
- INFERRED: 0 (0%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*